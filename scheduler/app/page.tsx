...
export default function Home() {
  ...
  return (
    <main className="min-h-screen w-full p-4 md:p-6 lg:p-8 flex flex-col items-center justify-center bg-background">
      <motion.div
        initial={{ opacity: 0, y: -20 }}
        animate={{ opacity: 1, y: 0 }}
        className="text-center mb-8"
      >
        <h1 className="text-3xl md:text-4xl font-bold mb-3">CEITBA | Combinador de Horarios</h1>
        <p className="text-muted-foreground">Selecciona tu carrera para comenzar a planificar tu horario</p>
      </motion.div>
      ...
      <div className="flex flex-col items-center space-y-8 w-full">
              ...
              <Link href={`/${career.id}?plan=${career.latestPlan}`}>
                <div className="group relative overflow-hidden rounded-xl p-4 bg-surface hover:shadow-lg transition-all duration-300 border border-border min-h-[100px] flex flex-col justify-between">
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <h2 className="text-lg font-semibold leading-tight">{career.name}</h2>
                  </div>
        ...
        >
          <Link href={`/${EXCHANGE_CAREER.id}?plan=${getLatestPlan(EXCHANGE_CAREER.id)}`}>
            <div className="group relative overflow-hidden rounded-xl p-4 bg-surface hover:shadow-lg transition-all duration-300 border-2 border-dashed border-primary/30 hover:border-primary/50">
              <div className="flex items-center justify-center space-x-3">
                ...
                <h2 className="text-lg font-semibold leading-tight">{EXCHANGE_CAREER.name}</h2>
                <svg
